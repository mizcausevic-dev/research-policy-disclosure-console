...
export function toSummary(report: CoverageReport) {
  ...
  return `${posture} ${report.projects} projects · ${report.packets} packets · ${report.highSeverityPackets} high-severity packets · ${report.workflowGaps} workflow gaps`;
}
...
export function toMarkdown(report: CoverageReport) {
  ...
  return [
    `# Research disclosure posture ${report.ok ? "✅" : "❌"}`,
    "",
    `- Projects: **${report.projects}**`,
    `- On-track projects: **${report.onTrackProjects}**`,
